...
export const loadPriceData = () => async (dispatch) => {
  ...
  let {data} = await xhr.get("https://min-api.cryptocompare.com/data/histoday?fsym=LIND&tsym=USD&limit=10");
  dispatch(setPriceData(data.Data));

  let {data: volumeData} = await xhr.get("https://min-api.cryptocompare.com/data/exchange/histohour?fsym=LIND&tsym=USD&limit=10");
