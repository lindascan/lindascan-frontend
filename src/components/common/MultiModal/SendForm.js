  ...
  token20Send = async () => {
    ...
    if(IS_MAINNET) {
        if (this.props.wallet.type === "ACCOUNT_LEDGER") {
            ...
            transactionId = await transactionResultManager(unSignTransaction, lindaWeb)
  ...
  renderFooter() {
    ...
    return (
        ...
        <button
            ...
            onClick={onClose}>{tu("lrc20_cancel")}</button>
          <button
              type="button"
              disabled={!this.isValid() || isLoading}
              className="btn btn-primary"
              onClick={this.onMultiSignSend}>{tu("lrc20_confirm")}</button>
        </Fragment>
    )
  }
